  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
    <DialogContent className="dashboard-customer-dialog" title={mode === 'paid' ? 'Pedidos pagos' : 'Clientes que abandonaram'}
      ...
      {mode === 'abandoned' ? <div className="customer-tabs" role="group" aria-label="Tipo de abandono">
        <Button aria-pressed={kind === 'draft'} variant={kind === 'draft' ? 'primary' : 'ghost'} onClick={() => { setKind('draft'); setPage(1); }}>Checkout incompleto · {drafts}</Button>
        <Button aria-pressed={kind === 'expired'} variant={kind === 'expired' ? 'primary' : 'ghost'} onClick={() => { setKind('expired'); setPage(1); }}>Pix expirado · {expired}</Button>
      </div> : null}
      <form className="customer-search" onSubmit={e => { e.preventDefault(); setSearch(term.trim()); setPage(1); }}>
      ...
      {query.isPending ? <Loading label="Carregando clientes…" /> : query.error ? <ErrorState message={descreverErro(query.error)} onRetry={() => void query.refetch()} /> : query.data ? <>
        ...
        <div className="customer-pagination"><Button variant="ghost" disabled={page === 1} onClick={() => setPage(p => p - 1)}>Anterior</Button><span>Página {page} de {Math.max(1, Math.ceil(query.data.total / query.data.size))}</span><Button variant="ghost" disabled={page * query.data.size >= query.data.total} onClick={() => setPage(p => p + 1)}>Próxima</Button></div>
      </> : null}
