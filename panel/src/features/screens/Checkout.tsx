...
import { ExternalLink, LayoutTemplate, ShoppingCart } from 'lucide-react';
...
import { Badge, Callout, Divider } from '@/components/ui/layout';
...
function OpcaoModo({
  ...
  return (
    <button
      ...
      aria-pressed={ativo}
      className={cn(
        'flex min-h-24 flex-col items-start gap-1.5 rounded-md border p-4 text-left transition-colors inset-shadow-hi',
        ativo
...
function Formulario({ inicial }: { inicial: CheckoutCfg }) {
  ...
  const sujo = JSON.stringify(corpo) !== JSON.stringify(inicial);

  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        salvar.mutate({ checkout: corpo });
      }}
    >
      <Surface as="section" tone="base" className="mb-5 max-w-[52rem]">
        ...
        <SaveBar
          ...
          saveLabel="Salvar checkout"
          message={sujo ? 'Alterações não salvas.' : 'Checkout salvo.'}
        />
      </Surface>
    </form>
  );
